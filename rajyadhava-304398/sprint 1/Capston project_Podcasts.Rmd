...
Data source: https://www.kaggle.com/listennotes/all-podcast-episodes-published-in-december-2017
Date: July 24 2019
...
pub_date:- Published date of this podcast
...
review=read.csv("podcast.csv", stringsAsFactors = FALSE)
names(review)
```
...
```{r}
corpus_review=Corpus(VectorSource(review$Text))
```
...
```{r}
corpus_review=tm_map(corpus_review, removeWords,c("also", "get","I", "this", "company","Inc", "can","just","that", "i","my","time","know","and","the","have","other"))
```
...
```{r}
##Create bi-grams
review_bigram <- tokens(review$Text) %>%
...
```{r}
# Tokenize descriptions
reviewtokens=tokens(review$Text, what="word",
...
```{r}
##Add the Y variable Recommend.IND
reviewtokensdf=cbind(review$Score,df)
head(reviewtokensdf)
```
...
#In sync with the CART model, the varimp plot of the Random forest model also indicates that “return”, “disappoint” are the most important variables. Most other words like “unfortunate”, “sad”, etc also indicate that these negative words are typically used by unhappy customers.  Inspite of the all the words appear on the text the most high_recommends and High are the most.  Hence the overall rating would be that there are more satisfied customers compared to the some unsatisfied. Hence the recommended customers would be coming back.

##Here is the URL of the shiny app https://rajkumaryadhav.shinyapps.io/rajkumaryadhav/

##Here in the above link of the shiny app we have shown the details of Exploratory text analysis.
